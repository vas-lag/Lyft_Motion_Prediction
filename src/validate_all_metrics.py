...
average_displacement_error_mean, final_displacement_error_oracle, final_displacement_error_mean)

import os
from collections import OrderedDict, defaultdict

from utils.uncompressed_dataset import UncompressedAgentDataset, MTPDataset
...
cfg = {
    ...
    'model_params': {
        ...
        'lr': 5e-5,
        'weight_path': "model_mobilenet_v2_mtp_output_cosine_annealing_5e_5_310000.pth",   
        'train': False,
        'predict': False,
        'visualize' : True
    },
...
if __name__ == "__main__":
    ...
    val_dataset = MTPDataset('../validation_compressed')
    val_dataloader = DataLoader(val_dataset, shuffle=val_cfg["shuffle"], batch_size=val_cfg["batch_size"], num_workers=val_cfg["num_workers"],
                                pin_memory=True, worker_init_fn=my_init_fn)
    ...
    metrics = [neg_multi_log_likelihood, time_displace, average_displacement_error_oracle, average_displacement_error_mean,
               final_displacement_error_oracle, final_displacement_error_mean, cross_track_error, along_track_error,
               average_cross_error_oracle, average_cross_error_mean, average_along_error_oracle, average_along_error_mean]
    ...
    df_dict = {metric_name: np.mean(values, axis=0) for metric_name, values in metrics_dict.items()}
    for metric_name, values in df_dict.items():
        if isinstance(values, np.float64):
            df_dict[metric_name] = [values] + [0] * 49
    df = pd.DataFrame(df_dict)
    df.to_csv(f"all_validation_metrics_{cfg['model_params']['weight_path'][:-4]}_test.csv")
