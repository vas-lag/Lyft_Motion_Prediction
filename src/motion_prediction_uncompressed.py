# -*- coding: utf-8 -*-
"""
Created on Thu Jun  3 21:44:54 2021
...
def visualize_trajectory(data, rasterizer, preds, confs, title="target_positions movement with draw_trajectory"):
    plt.figure(figsize = (8,6))
    im = data["image"][:, :, :].transpose(1, 2, 0)
    im = rasterizer.to_rgb(im)
    idx = np.argmax(confs.numpy())
    preds = preds.numpy()[idx]
    print(preds)
    target_positions_pixels = transform_points(preds + data["centroid"][:2], data["world_to_image"])
    draw_trajectory(im, target_positions_pixels, TARGET_POINTS_COLOR, radius=1, yaws=None)
...
class LyftMultiModel(nn.Module):
    ...
    def __init__(self, cfg: Dict, num_modes=3):   #num_modes=3
        ...
        elif architecture == "efficientnet_b0" or architecture == "mobilenet_v2":
...
def forward(data, model, device, criterion = pytorch_neg_multi_log_likelihood_batch):  #  pytorch_average_displacement_error_batch
...
cfg = {
    ...
    'model_params': {
        'model_architecture': 'efficientnet_b0',   #resnet34
        ...
        'model_name': "model_resnet18_no_semantic",    #model_resnet34_output
